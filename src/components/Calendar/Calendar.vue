<template>
  <div class="hx-calendar" :style="{ width: props.options.width, height: props.options.height }">
    <div class="hx-calender-setting">
      <div class="hx-calender-date">{{ currentDateString }}</div>
      <div class="hx-calender-selector">
        <div class="prev" @click="selectDate('prev-month')">&larr;</div>
        <div class="current">{{ currentMonth }}月</div>
        <div class="next" @click="selectDate('next-month')">&rarr;</div>
      </div>
    </div>
    <table class="hx-calendar-core">
      <thead>
        <tr class="hx-calendar-header">
          <th v-for="d in weekHeadDays" :key="d" class="hx-calendar-header-item">{{ d }}</th>
        </tr>
      </thead>
      <tbody>
...
const selectDate = (type: CalendarDateType) => {
  const dateMap: Record<CalendarDateType, Dayjs> = {
    ...
    'select-date': nextYearDayjs.value,
    today: dayjs()
  }
